...
const EXCLUDE_DIRS = new Set([
  ...
  "scripts",  // avoid including the scanner scripts
]);
...
// ─────────────────────────────────────────────────────────────────────
//  GITIGNORE PARSER — Reads .gitignore and converts patterns to matchers
// ─────────────────────────────────────────────────────────────────────
...
/** Convert a gitignore glob pattern into a RegExp */
function gitignorePatternToRegex(pattern: string): RegExp {
  ...
  // Remove trailing / (we handle directoryOnly separately)
  ...
  // If pattern has no slash (other than removed leading /), it matches
  // any file/dir with that name at any depth
  if (!anchored && !p.includes("/")) {
    return new RegExp(`(^|/)${regex}(/|$)`);
  }

  // Otherwise anchored to root
  return new RegExp(`^${regex}(/|$)`);
}

/** Parse a .gitignore file and return an array of rules */
function parseGitignore(rootDir: string): GitignoreRule[] {
...
function isGitignored(relPath: string, isDirectory: boolean, rules: GitignoreRule[]): boolean {
...
function walkProject(rootDir: string, gitignoreRules: GitignoreRule[] = []): WalkResult {
  const files: string[] = [];
  ...
  function walk(dirPath: string): void {
    ...
    for (const entry of entries) {
      ...
      const relPath = toPosix(path.relative(rootDir, full));

      if (stat.isDirectory()) {
        if (EXCLUDE_DIRS.has(entry)) continue;
        // Check gitignore
        if (isGitignored(relPath, true, gitignoreRules)) continue;
        walk(full);
      } else if (stat.isFile()) {
        ...
        // Check gitignore
        if (isGitignored(relPath, false, gitignoreRules)) continue;
...
// ─────────────────────────────────────────────────────────────────────
//  MARKDOWN GENERATOR — project_combined.md
// ─────────────────────────────────────────────────────────────────────
//
//  Generates a comprehensive markdown file containing:
//    1. Project Structure — visual tree with ├──/└── connectors
//    2. Project Paths     — flat list of absolute paths
//    3. File Contents     — full source code of every text file
//
//  This gives AI agents and humans a single-file snapshot of the
//  entire project — structure + code — in a portable format.

/** Build a visual tree string from the graph nodes */
function buildTree(graph: ProjectGraph): string {
  // Separate dirs and files, group by parent
  interface TreeEntry {
    id: string;
    label: string;
    type: "file" | "directory";
  ...
  const lookup = new Map<string | "__root__", TreeEntry[]>();
  ...
  for (const n of graph.nodes) {
    if (n.type === "directory") {
      ...
      if (!lookup.has(parentKey)) lookup.set(parentKey, []);
      lookup.get(parentKey)!.push({
        id: n.id,
        label: n.label + "/",
        type: "directory",
        children: [],
  ...
  // Second pass: create all file entries
  for (const n of graph.nodes) {
    if (n.type === "file") {
      const parentKey = n.parent ?? "__root__";
      if (!lookup.has(parentKey)) lookup.set(parentKey, []);
  ...
  function attachChildren(entry: TreeEntry): void {
    const kids = lookup.get(entry.id) ?? [];
    ...
    for (const child of kids) {
      if (child.type === "directory") attachChildren(child);
    }
  }

  const roots = lookup.get("__root__") ?? [];
  roots.sort((a, b) => {
    if (a.type !== b.type) return a.type === "directory" ? -1 : 1;
    return a.label.localeCompare(b.label);
  });
  for (const r of roots) {
    if (r.type === "directory") attachChildren(r);
  }

  // Render tree with connectors
  const lines: string[] = ["."]; // root

  function render(entries: TreeEntry[], prefix: string): void {
    for (let i = 0; i < entries.length; i++) {
      const isLast = i === entries.length - 1;
      const connector = isLast ? "└── " : "├── ";
      const childPrefix = isLast ? "    " : "│   ";

      lines.push(prefix + connector + entries[i].label);

      if (entries[i].children.length > 0) {
        render(entries[i].children, prefix + childPrefix);
      }
    }
  }
...
function fenceLanguage(ext: string): string {
  const map: Record<string, string> = {
    ".ts": "ts", ".tsx": "tsx", ".js": "js", ".jsx": "jsx",
    ".mjs": "js", ".cjs": "js", ".mts": "ts",
    ...
    ".env": "txt", ".properties": "properties",
    ".gradle": "gradle", ".ini": "ini", ".cfg": "ini",
    ".proto": "protobuf", ".csv": "csv",
  };
  return map[ext] ?? "txt";
}

/** Generate the project_combined.md markdown */
function generateMarkdown(graph: ProjectGraph, rootDir: string): string {
  const sections: string[] = [];

  // ── Section 1: Project Structure (visual tree) ──
  sections.push("# Project Structure\n");
  sections.push("```txt");
  sections.push(buildTree(graph));
  sections.push("```\n");

  // ── Section 2: Project Paths (absolute paths) ──
  sections.push("# Project Paths\n");
  sections.push("```txt");

  // Collect all paths (dirs with trailing \, files without), sorted
  const allPaths: string[] = [];
  for (const n of graph.nodes) {
    if (n.type === "directory") {
      allPaths.push(n.fullPath + path.sep);
    } else {
      allPaths.push(n.fullPath);
    }
  }
  allPaths.sort((a, b) => a.localeCompare(b));
  sections.push(allPaths.join("\n"));
  sections.push("```\n");

  // ── Section 3: File Contents ──
  sections.push("# File Contents\n");

  // Only include text files that we can read
  const fileNodes = graph.nodes
    .filter(n => n.type === "file" && n.ext && TEXT_EXTENSIONS.has(n.ext))
    .filter(n => (n.size ?? 0) <= MAX_FILE_BYTES)
    .sort((a, b) => a.fullPath.localeCompare(b.fullPath));

  for (const node of fileNodes) {
    const content = readSafe(node.fullPath);
    if (content === null) continue;

    const lang = fenceLanguage(node.ext ?? "");
    sections.push(`## File: ${node.fullPath}\n`);
    sections.push("```" + lang);
...
function main(): void {
  ...
  const gitignoreRules = parseGitignore(rootDir);
  if (gitignoreRules.length > 0) {
    console.log(`  Gitignore: ${gitignoreRules.length} rules loaded`);
  }

  // Walk the project tree
  console.log("  Scanning project structure...");
  const walkResult = walkProject(rootDir, gitignoreRules);
  ...
  const mdPath = path.join(outputDir, "project_combined.md");
  const md = generateMarkdown(graph, rootDir);
